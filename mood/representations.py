...
def representation_iterator(smiles, n_jobs: Optional[int] = None, progress: bool = True):
    for name in MOOD_REPRESENTATIONS:
        yield name, featurize(smiles, name, n_jobs, progress)
...
def featurize(
    ...
    reprs = dm.utils.parallelized(
        fn, smiles, n_jobs=n_jobs, progress=progress
    )
    
    return np.array(reprs)


def compute_whim(smi):
    """
...
_REPR_TO_FUNC = {   
    "MACCS": partial(dm.to_fp, fp_type="maccs"), 
    "ECFP6": partial(dm.to_fp, fp_type="ecfp", radius=3),
